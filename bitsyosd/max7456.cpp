...
void
OSD::writexy(uint8_t x, uint8_t y, uint8_t c) {
 openSingle(x, y);
 write(c);
}
...
// Access to 
void OSD::writeach(char* text, int s) {
  int i = 0;
  while (text[i]) {
    char n = text[i];
    // uppercase
    ...
    if(n>=48&&n<=57) {
      ...
      switch(n) {
        ...
        case 32: n = 0; break;
        // .
        case 46: n = 0x4a; break;
        // ,
        case 44: n = 0x49; break;  
        // - 
        case 45: n = 0x91; break; 
        // +
        case 43: n = 0x90; break;
        // a-z    
        default:  n = n-55 + s; break;
      }
    }
    write(n);
    i++;
...
void OSD::writeupper(char* text) {
  writeach(text, 1);
}

void OSD::writelower(char* text) {
  writeach(text, 37);  
}

void OSD::writedec(int d) {
  char tmp[255];
  writeupper(itoa(d, tmp, 10));
}

void OSD::writefloat(float d, int prec) {
  char tmp[255];
  writelower(dtostrf(d, 1, prec, tmp));
}
